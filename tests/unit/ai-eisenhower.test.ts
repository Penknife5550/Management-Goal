...
import {
  baueKlassifizierungsPrompt,
  jobKey,
  parseKiAntwort,
} from "../../src/lib/ai-eisenhower";
...
describe("parseKiAntwort", () => {
  it("akzeptiert eine gueltige Antwort und leitet den Quadranten ab", () => {
    const r = parseKiAntwort({ important: true, urgent: false, confidence: 0.78, reasoning: "Strategisch." });
  ...
  it("leitet alle vier Quadranten korrekt ab", () => {
    expect(parseKiAntwort({ important: true, urgent: true, confidence: 1, reasoning: "x" }).quadrant).toBe(1);
    expect(parseKiAntwort({ important: false, urgent: true, confidence: 0, reasoning: "x" }).quadrant).toBe(3);
    expect(parseKiAntwort({ important: false, urgent: false, confidence: 0.5, reasoning: "x" }).quadrant).toBe(4);
  });

  it("kappt ueberlanges reasoning und trimmt", () => {
    const lang = "  " + "a".repeat(600) + "  ";
    expect(parseKiAntwort({ important: true, urgent: true, confidence: 0.5, reasoning: lang }).reasoning.length).toBe(500);
  });

  it("wirft bei fehlenden/falschen Flags", () => {
    expect(() => parseKiAntwort({ urgent: false, confidence: 0.5, reasoning: "x" })).toThrow();
    expect(() => parseKiAntwort({ important: "ja", urgent: false, confidence: 0.5, reasoning: "x" })).toThrow();
  });

  it("wirft bei confidence ausserhalb 0..1", () => {
    expect(() => parseKiAntwort({ important: true, urgent: true, confidence: 2, reasoning: "x" })).toThrow();
    expect(() => parseKiAntwort({ important: true, urgent: true, confidence: -0.1, reasoning: "x" })).toThrow();
  });

  it("wirft bei leerem reasoning und bei Nicht-Objekt", () => {
    expect(() => parseKiAntwort({ important: true, urgent: true, confidence: 0.5, reasoning: "  " })).toThrow();
