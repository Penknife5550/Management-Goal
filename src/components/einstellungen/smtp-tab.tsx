...
import { Banner, BTN, BTN_SEK, behandleFehler, Feld, INPUT, type SmtpConfig, useAdminAction } from "./shared";
...
export function SmtpTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      {!config.isActive && (
        <Banner ton="warnung">SMTP ist nicht aktiv — es werden derzeit keine E-Mails versendet.</Banner>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Feld label="Port">
          <input className={INPUT} type="number" min={1} max={65535} value={config.port} onChange={(e) => set("port", Number(e.target.value) || 0)} />
        </Feld>
        ...
        <Feld label="Absender-Adresse (From)">
          <input className={INPUT} type="email" value={config.fromEmail} onChange={(e) => set("fromEmail", e.target.value)} placeholder="cockpit@credo-gruppe.de" />
        </Feld>
        ...
        <Feld label="Antwort-an (Reply-To, optional)">
          <input className={INPUT} type="email" value={config.replyToEmail} onChange={(e) => set("replyToEmail", e.target.value)} />
        </Feld>
